...
Connection::Connection(EventLoop *loop, Socket *sock)
    : _sock(sock), _loop(loop), _channel(nullptr), _inBuffer(new std::string()),
      _readBuffer(nullptr) {
  ...
  _channel->setReadCallback(cb); // 绑定回调函数
...
void Connection::send(int sock) {
  char buf[_readBuffer->size()];
  strcpy(buf, _readBuffer->c_str());
  int data_size = _readBuffer->size();
  int data_left = data_size;
  while (data_left > 0) {
    ssize_t bytes_write = write(sock, buf + data_size - data_left, data_left);
